...
export const rowPassesFilters = (filters, row) => {
  for (let column in filters) {
    ...
    if (filter.filterType === FILTER_TYPE.NUMERIC && !rowPassesNumericFilter(filter, row)) {
      return false;
    }

    if (filter.filterType === FILTER_TYPE.STRING && !rowPassesStringFilter(filter, row)) {
      return false;
    }
  }
...
export const createNumericFilter = (column, enabled=false, min=0, max=Number.MAX_SAFE_INTEGER) => {
  return {
